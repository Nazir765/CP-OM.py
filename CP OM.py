...
def login_lagi334():
	try:
		...
		print('\n LOGIN BERHASIL  --->jalankan ulang ketik python cp.py ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	io = '''[01] PUBLIC FRIENDS	     [06] CRACK FROM FILES
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='[bold green]LIVE[/bold green]'))
					ok+=1
					break
